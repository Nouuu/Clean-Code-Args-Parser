...
export class Args implements Parser {
    ...
    schema.split(',').forEach((keyArg) => {
      switch (keyArg.trim()[1]) {
        case tokenArgs.booleanToken:
          this.booleanKeys.set(keyArg, this.defaultValues.defaultBoolean);
          break;
  ...
  private parseString(value: string, key: string) {
    if (
      ...
      throw new ParsingError(`Cannot parse string (${value}) on key ${key}`);
    }
  }
  ...
  private parseNumber(value: string, key: string) {
    ...
    if (isNaN(number)) {
      throw new ParsingError(`Cannot parse number (${value}) on key ${key}`);
    }
